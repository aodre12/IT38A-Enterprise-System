...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_verify()) { $error = 'Invalid request.'; }
        ...
        if ($name && $role) {
            $s = $conn->prepare("INSERT INTO personnel (name, role, status) VALUES (?, ?, ?)");
            $s->bind_param("sss", $name, $role, $status);
            if ($s->execute()) { header('Location: dashboard.php'); exit; }
            else $error = "Failed to add personnel.";
...
<title>Add Personnel | UtilityTrack</title>
...
.form-group input,.form-group select{width:100%;padding:10px 12px;border:1px solid var(--border);border-radius:8px;font-size:.9rem;transition:border-color .2s;}
.form-group input:focus,.form-group select:focus{outline:none;border-color:var(--accent);}
.form-error{color:var(--danger);font-size:.85rem;margin-bottom:14px;}
</style></head><body>
<?php include __DIR__.'/sidebar.php'; ?>
<div class="topbar">
  <div style="display:flex;align-items:center;gap:14px;"><button class="hamburger" id="hamburger"><i class="fas fa-bars"></i></button><span class="page-title">Add Personnel</span></div>
...
<main class="main">
  <div class="section-card" style="max-width:520px;">
    <div class="section-header"><h2><i class="fas fa-user-plus"></i> Add New Personnel</h2></div>
    <div class="section-body">
      <?php if($error): ?><div class="form-error"><i class="fas fa-exclamation-circle"></i> <?= htmlspecialchars($error) ?></div><?php endif; ?>
      <form method="POST">
        <?= csrf_field() ?>
        <div class="form-group"><label>Full Name</label><input type="text" name="name" placeholder="e.g. Juan dela Cruz" required></div>
        <div class="form-group"><label>Role</label><input type="text" name="role" placeholder="e.g. Technician" required></div>
        ...
        <button type="submit" class="btn btn-success" style="width:100%;justify-content:center;padding:11px;"><i class="fas fa-save"></i> Add Personnel</button>
      </form>

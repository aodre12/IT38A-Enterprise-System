...
if (!$personnel) {
    echo "<h2 style='color:red; text-align:center; margin-top:50px;'>Personnel not found.</h2>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $role) {
        $updateStmt = $conn->prepare("UPDATE personnel SET name=?, role=?, status=? WHERE id=?");
        $updateStmt->bind_param("sssi", $name, $role, $status, $id);
...
<head>
    <meta charset="UTF-8" />
    <title>Edit Personnel - Utility ERP</title>
    <style>
...
<div class="container">
    ...
    <form method="POST">
        ...
        <input type="text" id="name" name="name" value="<?= htmlspecialchars($personnel['name']) ?>" required />

        <label for="role">Role:</label>
        <input type="text" id="role" name="role" value="<?= htmlspecialchars($personnel['role']) ?>" required />

        <label for="status">Status:</label>
        <select id="status" name="status">
            <option value="Active" <?= $personnel['status'] === 'Active' ? 'selected' : '' ?>>Active</option>
            <option value="Inactive" <?= $personnel['status'] === 'Inactive' ? 'selected' : '' ?>>Inactive</option>
        </select>
